...
public class ASymBinarizerEvalScoreDist implements Serializable {
	...
	public ASymBinarizerEvalScoreDist(final TernaryOutputNeuron neuron, boolean forcePosNeg) {
	...
	public TernarySolution[] findBestBinarizedConfiguration(ScoreFunctions[] scoreFuns, BufferedWriter[] bw) {
		...
		return this.binarizationQuality.entrySet().parallelStream()
				.map(new Function<Entry<TwPair, TreeMap<Integer, TernaryProbDistrib>>, TernarySolution[]>() {
					...
					public TernarySolution[] apply(Entry<TwPair, TreeMap<Integer, TernaryProbDistrib>> t) {
						...
						for (int i = 0; i < scoreFuns.length; i++) {
							TernarySolution sol = currentBestParams[i];
							synchronized (bw[i]) {
								try {
									bw[i].write(sol.twPos + "," + sol.twNeg + "," + sol.twPosIndex + ","
											+ sol.twNegIndex + "," + sol.th + "," + sol.tl + "," + sol.score + "\n");
								} catch (IOException e) {
									e.printStackTrace();
								}
							}
	...
	public static void main(String[] args) throws Exception {
		final int neuronIndex = Integer.parseInt(args[4]);
		String trainingData = args[0];
		String weightsData = args[1];
		String biasData = args[2];
		String outputFile = args[3] + "_neuron_" + neuronIndex;
		// long start = System.currentTimeMillis();
		ASymBinarizerEvalScoreDist binarizer = new ASymBinarizerEvalScoreDist(
		...
		List<boolean[]> inputData = FilesProcessing.getFilteredTrainingSet(trainingData, Integer.MAX_VALUE);
		System.out.println("loaded input data: " + inputData.size());
		inputData.parallelStream().forEach(input -> {
			binarizer.update(input);
			int nbDone = done.incrementAndGet();
			if (nbDone % 100 == 0) {
				System.out.println("done " + nbDone);
			}
		});
		...
		// s");
		System.out.println(Arrays.toString(binarizer.findBestBinarizedConfiguration(ScoreFunctions.values(), bw)));
